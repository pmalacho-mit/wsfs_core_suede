...
from ...sqlmodel_utils_suede.associations import now
from ...sqlmodel_utils_suede.postgres.db import Database
...
async def initialize_within(
    ...
    async with submitting(backend, controller, user_id) as submission:
        ...
        reconciliation.events.extend(await service.resolve_name_collisions(submission))
...
async def apply_within(
    ...
    async with submitting(backend, controller, user_id) as submission:
        ...
        events = [*outcome.events, *await service.resolve_name_collisions(submission)]
        await submission.session.commit()
...
WARMING_AT_ONCE = 8
"""How many rooms are filled concurrently.
...
that has an opinion about that. Not tuned; small enough to be polite and large
enough that the wall clock is not the sum.
"""

WARMING_GATE = asyncio.Semaphore(WARMING_AT_ONCE)
"""That bound, held ACROSS THE PROCESS rather than per call.
