...
from ...sqlmodel_utils_suede.associations import (
    ID,
    WithID,
    WithTime,
    ForeignKeyField,
...
from ...sqlmodel_utils_suede.columns import EnumField
from ...sqlmodel_utils_suede.tablenames import tablename
...
def table_of(named: str | type[WithTableName]) -> str:
    """A host's table, however they care to name it.

    PUBLIC API. A satellite package building tables of its own against the
    same host takes the same two arguments `build_models` does and needs the
    same answer from them, so this is exported rather than copied.
    """
...
def build_models(
    *,
    ...
    """
    users, workspaces = table_of(user_table), table_of(workspace_table)
